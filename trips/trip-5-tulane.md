# Trip 5 — vs Tulane Green Wave (HOME)
**Status: PLANNED — March 23, 2026**

---

```
═══════════════════════════════════════════════════════════════
  ARMY BLACK KNIGHTS TRAVEL PLAN
  Trip 5 — vs Tulane Green Wave | HOME at Michie Stadium
═══════════════════════════════════════════════════════════════

GAME DETAILS
────────────────────────────────────────────────────────────────
  Opponent    : Tulane Green Wave
  Date        : Saturday, October 10, 2026
  Kickoff     : TBD
  Stadium     : Michie Stadium, West Point, NY
  Location    : West Point, New York
  Game Type   : AAC Conference | Rematch of 2024 AAC Championship


FLIGHT OPTIONS — DFW ↔ EWR / LGA / JFK  ·  Out: Fri Oct 9  ·  Ret: Sun Oct 11
────────────────────────────────────────────────────────────────
  Carriers : American Airlines | United Airlines (both nonstop ~3h 33m)
  Rule     : Search EWR, LGA, and JFK — book cheapest nonstop across all airports

  OUTBOUND (DFW → EWR | LGA | JFK)
    1 ★  AA    ~7–9 AM CT    → ~12–2 PM ET at EWR    Nonstop  ~$159–210/pp
    2    UA    ~8–10 AM CT   → ~1–3 PM ET at EWR     Nonstop  ~$159–220/pp
    3    AA/UA  LGA or JFK — pick cheapest · add ~15–30 min to drive  ~$130–200/pp

  RETURN (EWR | LGA | JFK → DFW)
    1 ★  AA    ~2–4 PM ET    → ~5:30–7:30 PM CT      Nonstop  ~$159–210/pp

  ★ recommended · No return before 2:00 PM ET


GROUND TRANSPORT — NYC AIRPORT TO CENTRAL VALLEY / SUFFERN
────────────────────────────────────────────────────────────────
  Mode          : Rental car from whichever NYC airport you fly into
  From EWR → CS : NJ Tpke N → I-287 E → I-87 N → Exit 16
                  (~65 miles, ~1h 15m to Comfort Suites)
  From EWR → CP : NJ Tpke N → I-287 N → NY-17 N → Suffern
                  (~55 miles, ~1h to Crowne Plaza)
  From LGA/JFK  : GWB → I-87 N → Exit 16 (~55–65 mi, ~1h–1h 30m)
  Hotel → Stadium: ~15–25 min to Michie Stadium
  Rental return : Return to same airport on departure day
  Est. cost     : ~$80–110 for weekend rental


HOTEL OPTIONS
────────────────────────────────────────────────────────────────
  Check-in    : Friday, October 9, 2026
  Check-out   : Sunday, October 11, 2026
  Nights      : 2

  Option 1 (RECOMMENDED)
  ┌─────────────────────────────────────────────────────────┐
  │  Name        : Comfort Suites Central Valley – Woodbury │
  │  Chain       : Choice Hotels                            │
  │  Address     : 100 Estrada Rd, Woodbury, NY 10917       │
  │  Distance    : ~15 miles from Michie Stadium (~20 min)  │
  │  Est. Rate   : ~$130–160/night | Total: ~$260–320       │
  │  Parking     : Free on-site ✓                           │
  └─────────────────────────────────────────────────────────┘

  Option 2
  ┌─────────────────────────────────────────────────────────┐
  │  Name        : Crowne Plaza Suffern-Mahwah              │
  │  Chain       : IHG (Crowne Plaza)                       │
  │  Address     : Three Executive Blvd, Suffern, NY 10901  │
  │  Distance    : ~25 miles from Michie Stadium (~35 min)  │
  │  Est. Rate   : ~$150–185/night | Total: ~$300–370       │
  │  Parking     : On-site available                        │
  └─────────────────────────────────────────────────────────┘


COST SUMMARY
────────────────────────────────────────────────────────────────
  (Option 1 flights + Option 1 hotel, 3 passengers)

  Flights (RT)    : ~$320–420/person × 3 = ~$960–1,260
  Hotel           : ~$145/night × 2 nights = ~$290
  Rental Car      : ~$95 weekend
  Gas/Tolls       : ~$40
  ─────────────────────────────────────────────────
  ESTIMATED TOTAL : ~$1,385–1,685

  Budget Status   : ✅ WITHIN BUDGET


NOTES & RECOMMENDATIONS
────────────────────────────────────────────────────────────────
  - Columbus Day weekend (Oct 12) — check if rates are elevated
  - AAC Championship rematch — high-energy game atmosphere
  - Peak fall foliage in Hudson Valley; area will be busy
    with tourists; book hotel early

═══════════════════════════════════════════════════════════════
  Generated   : March 23, 2026
  Data source : SerpAPI web search + hotel web search
  Prices are estimates — verify current rates before booking.
═══════════════════════════════════════════════════════════════
```
