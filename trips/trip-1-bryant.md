# Trip 1 — vs Bryant Bulldogs (HOME)
**Status: BOOKED — Flights (Delta GV8589, DFW↔LGA) | Hotel (Comfort Suites Central Valley) | Rental car (Hertz L5403844789, LGA)**

---

```

GAME DETAILS
────────────────────────────────────────────────────────────────
  Opponent    : Bryant Bulldogs
  Date        : Saturday, September 5, 2026
  Kickoff     : TBD (likely noon or 3:30 PM ET — season opener)
  Stadium     : Michie Stadium, West Point, NY
  Location    : West Point, New York


FLIGHTS ✅ CONFIRMED — Delta
────────────────────────────────────────────────────────────────
  Out  Fri Sep 4   DL551   DFW  7:00 AM CT  →  LGA  11:32 AM ET

  Ret  Sun Sep 6   DL352   LGA  6:45 PM ET  →  DFW   9:49 PM CT


RENTAL CAR ✅ CONFIRMED — Hertz
────────────────────────────────────────────────────────────────
  Volvo XC90 Hybrid
  Pickup   LGA  Fri Sep 4   12:30 PM
  Return   LGA  Sun Sep 6    4:00 PM


HOTEL ✅ CONFIRMED — Comfort Suites Central Valley


ACTIVITIES
────────────────────────────────────────────────────────────────
  West Point Museum          Free · on-campus · walk from Michie Stadium
  Trophy Point overlook      Scenic Hudson River views on West Point grounds
  Jones Farm (Sun breakfast) 190 Angola Rd, Cornwall, NY · Cafe 8am–3pm · farm café + bakery overlooking Schunemunk Mtn
  Storm King Art Center      Outdoor sculpture park · ~15 min south (Cornwall, NY) · right next to Jones Farm
  Cold Spring, NY            Main Street shops + river views · ~20 min south
  Dia:Beacon                 Modern art museum · ~20 min north (Beacon, NY)
  Hudson Valley wineries     Brotherhood Winery, Millbrook Vineyards · 30–45 min east


COST SUMMARY
────────────────────────────────────────────────────────────────
  CONFIRMED TOTAL : ~$2,322  ·  ✅ WITHIN BUDGET
  Flights $1,344 ✅ · Hotel $264 ✅ · Rental $314.38 ✅ · DFW Pkg $75 · Food ~$300 · Tolls ~$25



```
